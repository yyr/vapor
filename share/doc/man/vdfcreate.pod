...
be accessed at varying, power-of-two coarsened approximations. Moreover,
two types of VDC files are supported: VDC1 and VDC2. The latter, VDC2,
supports level-of-detail access in addtion to hierarchical access.

=head1 OPTIONS

=over 4

=item -cratios E<lt>vector<gt>

This option (VDC 2 only) specifies a colon delimited list of
...
B<-wname>) and the block dimension (See B<-bs>).
...
should not be changed from the default value.
...
Generate a .vdf file for a VDC version 2 data collection.

=item -wname E<lt>wave_name<gt>
