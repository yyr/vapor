...
IsolineRenderer::IsolineRenderer(Visualizer* glw, RenderParams* pParams )
...
int IsolineRenderer::_paintGL(DataMgr* dataMgr)
{
	...
	int timestep = myVisualizer->getActiveAnimationParams()->getCurrentTimestep();

	if (!cacheIsValid(timestep) || myVisualizer->textRenderersAreDirty()){
		if (!buildLineCache(timestep)) return -1;
	}
...
int IsolineRenderer::_initializeGL()
{
...
bool IsolineRenderer::buildLineCache(int timestep){
	...
	for (int iso = 0; iso < isovals.size(); iso++){
		if(iParams->GetTextDensity() > 0. && iParams->textEnabled()) { //put a textObject in the GLWindow to hold annotation of this isovalue
			...
			int objNum = myVisualizer->addTextObject(this, GetAppPath("VAPOR","share",vec).c_str(),(int)iParams->GetTextSize(),lineColor, bgc,1, isoText);
