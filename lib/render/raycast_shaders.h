...
static char fragment_shader_iso_lighting[] =
  ...
  "  vec3 start = texture2D(texcrd_buffer, tex_crd_2d).xyz;\n"
  ...
  "  //float zstart = texture2D(depth_buffer, tex_crd_2d).x;\n"
  "  float zstart = shadow2D(depth_buffer, vec3(tex_crd_2d,0.0)).x;\n"
  "  float zlen = gl_FragCoord.z - zstart;\n"
  "  float zdelta = zlen / float(nsegs);\n"
  "  "
  "  vec3 texCoord0 = start;\n"
  ...
  "         vec3 isoTexCoord = texCoord0 + (weight * delta_dir);\n"
...
static char fragment_shader_iso[] =
  ...
  "  float delta1 = len / float(nsegs);\n"
  "  vec3 delta_dir = dir_unit * delta1;\n"
  "  //float zstart = texture2D(depth_buffer, vec3(tex_crd_2d,0.0)).x;\n"
  "  float zstart = shadow2D(depth_buffer, vec3(tex_crd_2d,0.0)).x;\n"
  "  float zlen = gl_FragCoord.z - zstart;\n"
  "  float zdelta = zlen / float(nsegs);\n"
  "  "
  ...
  "  float s1 = texture3D(volumeTexture, texCoord1).x;\n"
  "  //if (isovalues >1.0) {\n"
  "   // gl_FragColor = vec4(zstart, 0.0, 1.0, 1.0);\n"
  "    //return;\n"
  "  //}\n"
  "  "
  "  float fragDepth = zstart;\n"
  ...
  "  }\n"
  "  if (isovalues > 1.0) {\n"
  "    fragDepth = zstart + ((gl_FragCoord.z - zstart) * 0.5);\n"
