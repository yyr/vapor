...
int DCCF::GetMapProjection(string lon, string lat, string &projstring) const {

	string key = lon + ":" + lat;

	map <string, string>::const_iterator itr = _proj4Strings.find(key);
	if (itr == _proj4Strings.end()) {
		SetErrMsg(
			"Invalid horizontal coordinate pair: %s, %s",
			lon.c_str(), lat.c_str()
		);
		return(-1);
	}
	projstring = itr->second;
	return(0);
}
...
int DCCF::GetVar(string varname, float *data) {
	...
	bool ok = DCCF::GetBaseVarInfo(varname, var);
	if (!ok) {
		SetErrMsg("Invalid variable name : %s", varname.c_str());
		return(-1);
	}
...
int DCCF:: _get_latlon_coordvars(
	NetCDFCFCollection *ncdfc, string dvar, string &loncvar, string &latcvar
) const {
	loncvar.clear();
	latcvar.clear();
...
Proj4API *DCCF::_create_proj4api(
	...
	string lon, string lat, string &proj4string
) {
	proj4string.clear();

	float lonmin, lonmax;
	int rc = _get_latlon_extents(ncdfc, lon, true, lonmin, lonmax);
	if (rc < 0) return(NULL);

	float latmin, latmax;
	rc = _get_latlon_extents(ncdfc, lon, false, latmin, latmax);
	if (rc < 0) return(NULL);
...
int DCCF::_AddCoordvars(
	...
	for (int i=0; i<cvars.size(); i++) {
		...
		_coordVarsMap[cvars[i]] = CoordVar(
			cvars[i], dims, units, DC::FLOAT, periodic, axis, false
		);
	}
...
int DCCF::_InitHorizontalCoordinatesDerived(
	...
	for (int i=0; i<coordpairs.size(); i++) {
		...
		_coordVarsMap[name] = CoordVar(
			name, londims, "meters", DC::FLOAT, periodic, 0, false
		);
		...
		_coordVarsMap[name] = CoordVar(
			name, latdims, "meters", DC::FLOAT, periodic, 1, false
		);
	}
...
int DCCF::_InitVerticalCoordinatesDerived(
	...
	for (int i=0; i<cvars.size(); i++) {
		...
		//
		_coordVarsMap[name] = CoordVar(
			name, dims, "meters", DC::FLOAT, periodic, 2, false
		);
	}
...
int DCCF::_InitDimensions(
	...
	for (int i=0; i<dimnames.size(); i++) {
		...
		else if (ncdfc->IsLatCoordVar(dimnames[i])) {
			axis = 1;
		} 
		else if (ncdfc->IsVertCoordVar(dimnames[i])) {
			axis = 2;
		}
...
int DCCF::_GetVarCoordinates(
	...
	int rc = ncdfc->GetVarCoordVarNames(varname, coordvars);
	if (rc<0) return(-1);

	reverse(coordvars.begin(), coordvars.end());	// DC dimension order
	
	vector <size_t> dims = ncdfc->GetDims(varname);
	assert(dims.size() == coordvars.size());
	...
	for (int i=0; i<coordvars.size(); i++) {
		string xcv;
		if (ncdfc->IsLonCoordVar(coordvars[i])) {
			xcv = coordvars[i] + "X";
		}
		else if (ncdfc->IsLatCoordVar(coordvars[i])) {
			xcv = coordvars[i] + "Y";
		}
		else if (ncdfc->IsVertCoordVar(coordvars[i])) {
			xcv = coordvars[i] + "Z";
		}
		else if (ncdfc->IsTimeCoordVar(coordvars[i])) {
			xcv = coordvars[i] + "T";
		}
	...
	// Search _dimsMap for the dimensions
	//
	for (int j=0; j<dimnames.size(); j++) {
		assert(_dimsMap.find(dimnames[j]) != _dimsMap.end());
		dimensions.push_back(_dimsMap[dimnames[j]]);
	}
...
cout << "Var i " << i << " " << vars[i] << endl;
		...
		if ( ! (
			...
			NetCDFSimple::IsNCTypeInt(type))) continue; 

		vector <DC::Dimension> dimensions;
		vector <string> coordvars;

		int rc = _GetVarCoordinates(ncdfc, vars[i], dimensions, coordvars);
		if (rc<0) {
...
DCCF::DerivedVarHorizontal::DerivedVarHorizontal(
	...
	const vector <DC::Dimension> &dims, Proj4API *proj4API, bool xflag
) : DerivedVar(ncdfc) {
