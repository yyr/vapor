...
int NetCDF::Initialize(string path)
{
	_dimnames.clear();
	_dims.clear();
	_unlimited_dimnames.clear();
...
int NetCDF::OpenRead(
	string path, const NetCDF::Variable &variable
) {
	size_t chsz = _chsz;
	int ncid;
	int rc = nc__open(path.c_str(), NC_NOWRITE, &chsz, &ncid);
...
string NetCDF::DimName(int id) const {
	if (id>=0 && id < _dimnames.size()) return (_dimnames[id]);

	return("");
}

int NetCDF::DimId(string name) const {
...
namespace VAPoR {
std::ostream &operator<<(std::ostream &o, const NetCDF &nc) {
	o << "NetCDF" << endl;
...
vector <string> NetCDF::Variable::GetAttNames() const {
	vector <string> names;
...
int NetCDF::Variable::GetAttType(string name) const {
	for (int i=0; i<_flt_atts.size(); i++) {
...
void NetCDF::Variable::GetAtt(string name, vector <double> &values) const {
...
void NetCDF::Variable::GetAtt(string name, vector <long long> &values) const {
...
void NetCDF::Variable::GetAtt(string name, string &values) const {
...
bool NetCDF::Variable::Match(
	const NetCDF::Variable &v
) const {
...
namespace VAPoR {
std::ostream &operator<<(std::ostream &o, const NetCDF::Variable &var) {

	o << "Variable" << endl;
	o << " Name : " << var._name << endl;
	o << " NetCDF var id : " << var._varid << endl;
	o << " NetCDF type : " << var._type << endl;
	o << " Dimensions : " << endl;
