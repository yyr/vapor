...
int DCReaderGRIB::ReadSlice(float *values){
    ...
    if (_openVar == "absv"){//"ELEVATION") {
        for (int x=0; x<_Ni; x++){
            for (int y=0; y<_Nj; y++){
                values[x + y*_Ni] = _hydrostaticElevation[x + y*_Ni + (_pressureLevels.size()-_sliceNum-1)*_Ni*_Nj];
            }
        }
        _sliceNum--;
        return 1;
    ...
    //cout << _openVar << " " << _openTS << " " <<  usertime << " " << level << " " << _sliceNum << " " << offset << " " << filename << " " << min << " " << max <<
    delete [] _dvalues;
...
int DCReaderGRIB::DoHydroVarsExist() {
    ...
    if (_vars2d.find("pres") == _vars2d.end()) {
...
int DCReaderGRIB::LoadHydroData(float* T, float* Q, float* Sp) {
    if (OpenVariableRead(_openTS,"t") == -1)
        return -1;
    Read(T);

    if (OpenVariableRead(_openTS,"q") == -1)
        return -1;
    Read(Q);

    if (OpenVariableRead(_openTS,"sp") == -1)
    //if (OpenVariableRead(_openTS,"pres") == -1)
        return -1;
    if (ReadSlice(Sp) == -1) {
        MyBase::SetErrMsg("Error reading surface pressure values at timestep %i, slice %i",_openTS,_sliceNum);
        return -1;
    }
    return 1;
}

int DCReaderGRIB::CalcVirtualTemperature(float *T, float *Q, float *Sum, float *Tv) {
    // Calculate virtual temperature at the surface level
    int index, indexBelow;
    int Nz = _pressureLevels.size();
    float A, B, C, D;
    for (int y=0; y<_Nj; y++) {
        for (int x=0; x<_Ni; x++) {
            index = x + y*_Ni;
    ...
    // Calculate virtual temperature at all other points
    for (int z=1; z<Nz; z++) {
        for (int y=0; y<_Nj; y++) {
            for (int x=0; x<_Ni; x++) {
                index = x + y*_Ni + z*_Ni*_Nj;
                indexBelow = x + y*_Ni + (z-1)*_Ni*_Nj;
                A = T[index] * (1+.61*Q[index]);
                B = T[indexBelow] * (1+.61*Q[indexBelow]);
                C = log10(_pressureLevels[z-1] -_pressureLevels[z]);
                D = log10(_pressureLevels[0] - _pressureLevels[z]);
                Sum[index] = ((A+B)/(2*C));         // Average A and B, then normalize w.r.t the span of their pressure interval (C)
                Tv[index] = Sum[index];
                for(int zSum=1; zSum<Nz; zSum++) {
...
int DCReaderGRIB::HydrostaticPressureEqn() {
    int Nz = _pressureLevels.size();
    float *T = new float[_Ni*_Nj*Nz];
    float *Q = new float[_Ni*_Nj*Nz];
    float *Sp = new float[_Ni*_Nj];
    float *Tv = new float[_Ni*_Nj*Nz];
    float *Sum = new float[_Ni*_Nj*Nz];
    _hydrostaticElevation = new float[_Ni*_Nj*Nz];

    int rc = DoHydroVarsExist();
    if (rc == -1) {
        MyBase::SetErrMsg("ERROR: One or more variables are missing for Hydrostatic Equation.  Aborting.");
        return -1;
    }

    if (LoadHydroData(T, Q, Sp)==-1) {
        MyBase::SetErrMsg("ERROR: Unable to load parameters for Hydrostatic Equation.  Aborting.");
        return -1;
    }
    if (!CalcVirtualTemperature(T, Q, Sum, Tv)) {
        MyBase::SetErrMsg("ERROR: Unable to calculate Virtual Temperature.  Aborting.");
        return -1;
    }
    ...
    for (int z=0; z<Nz; z++) {
        ...
        for (int y=0; y<_Nj; y++) {
            for (int x=0; x<_Ni; x++) {
                ...
                A = R*Tv[index]/g;
                //B = _pressureLevels[0];
                B = Sp[x + y*_Ni]/100;  // divide by 100 to get hpa
                C = _pressureLevels[z];
                D = log10 ( B / C );
                _hydrostaticElevation[index] = A*D;
            }
        }
    }
...
double DCReaderGRIB::BarometricFormula(const double pressure) const {
    ...
    height = -1 * R * To * log10(pressure2/Po) / (g * M);
    return height;
}
...
int DCReaderGRIB::GribParser::_VerifyKeys() {
    ...
    for (int i=0; i<numRecords; i++) {
        ...
        if ((strcmp(grid.c_str(),"regular_ll")!=0) &&
            ...
            MyBase::SetErrMsg("Error: Invalid grid specification ('%s') for Record No. %f",grid.c_str(),i);
            return -1;
        }
