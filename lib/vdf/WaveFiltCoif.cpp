...
void WaveFiltCoif::_analysis_initialize (int member) 
{
  ...
  for (int count = 0; count < _filterLength; count++)
    _lowDecomFilCoef[count] *= sqrt(2.0);
  for (int count = 0; count < _filterLength; count++)
    _hiDecomFilCoef[count] *= sqrt(2.0);
...
void WaveFiltCoif::_synthesis_initialize (int member) 
{
  ...
  for (int count = 0; count < _filterLength; count++)
    _lowReconFilCoef[count] *= sqrt(2.0);
  for (int count = 0; count < _filterLength; count++)
    _hiReconFilCoef[count] *= sqrt(2.0);
