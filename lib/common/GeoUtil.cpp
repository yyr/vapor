...
//
template <class T>
void _ShiftLonTemplate(const T *srclon, int nx, T *dstlon) 
{
	dstlon[0] = srclon[0];
	double min = dstlon[0];
	double max = dstlon[0];
...
template <class T>
void _LonExtentsTemplate(
	const T *lon, int nx, int ny, 
	T &lonwest, T &loneast
) {
...
void _LatExtentsTemplate(
	const T *lat, int nx, int ny, 
	T &latsouth, T &latnorth
) {
...
void GeoUtil::ShiftLon(const float *srclon, int nx, float *dstlon)  {
...
void GeoUtil::LonExtents(
	const float *lon, int nx, int ny, 
	float &lonwest, float &loneast
) {
	_LonExtentsTemplate(lon, nx, ny, lonwest, loneast);
}

void GeoUtil::LonExtents(
	const double *lon, int nx, int ny, 
	double &lonwest, double &loneast
) {
	_LonExtentsTemplate(lon, nx, ny, lonwest, loneast);
}

void GeoUtil::LonExtents(
	const float *lon, int nx, 
	float &lonwest, float &loneast
) {
	_LonExtentsTemplate(lon, nx, lonwest, loneast);
}

void GeoUtil::LonExtents(
	const double *lon, int nx, 
	double &lonwest, double &loneast
) {
	_LonExtentsTemplate(lon, nx, lonwest, loneast);
}

void GeoUtil::LatExtents(
	const float *lat, int nx, int ny, 
	float &latsouth, float &latnorth
) {
	_LatExtentsTemplate(lat, nx, ny, latsouth, latnorth);
}

void GeoUtil::LatExtents(
	const double *lat, int nx, int ny, 
