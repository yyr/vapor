...
asciiPrintProduct(
    ...
    IdGetter			getId)
{
    int		nchar = _snprintf(buf, size, "%s", "");
    ...
    if (nchar >= 0) {
        ...
        for (i = 0; i < count && nchar >= 0; i++) {
            ...
            if (powers[i] != 1) {
                n = RETURNS_NAME(getId)
                    ? _snprintf(buf+nchar, size, "^%d", powers[i])
                    : _snprintf(buf+nchar, size, "%d", powers[i]);
        ...
        for (iBasic = 0; iBasic < count; iBasic++) {
            ...
            if (power != 0) {
                ...
                if (nchar > 0) {
                     ...
                     */
                    n = _snprintf(buf+nchar, size, "%s", "\xc2\xb7");
                ...
                if (power != 1) {
                    ...
                    if (power < 0) {
                         ...
                         */
                        n = _snprintf(buf+nchar, size, "%s", "\xe2\x81\xbb");
	...
	if (order == NULL) {
	    ...
	    getBasicOrder(powers, count, order, &positiveCount, &negativeCount);
            ...
            if (nchar >= 0 && (positiveCount + negativeCount > 0)) {
                ...
                if (positiveCount == 0) {
                    n = _snprintf(buf+nchar, size, "%s", "1");
                    if (0 > n) {
                        nchar = n;
                    }
                    else {
            ...
            nchar = 
                id == NULL
                    ? formatPar->printProduct(basicUnits, powers, count,
                        formatPar->buf, formatPar->size, formatPar->getId)
                    : _snprintf(formatPar->buf, formatPar->size, "%s", id);
	}
    }
    formatPar->nchar = nchar < 0 ? nchar : formatPar->nchar + nchar;
    ...
    const int               addParens)
{
    ...
    if (scale != 1) {
        ...
        n = _snprintf(buf, size, needParens ? "(%.*g " : "%.*g ", DBL_DIG,
                scale);
        if (0 > n) {
            nchar = n;
        }
    ...
    if (0 <= nchar) {
        ...
        if (n < 0) {
            ...
            if (offset != 0) {
                needParens = addParens;
                n = RETURNS_NAME(getId)
                    ? _snprintf(buf+nchar, size, " from %.*g", DBL_DIG,
                            offset)
                    : _snprintf(buf+nchar, size, " @ %.*g", DBL_DIG, offset);
                if (0 > n) {
                    nchar = n;
                }
                else {
    ...
    const int	        	addParens)
{
	...
	if (base == 2) {
	    nchar = _snprintf(buf, size, "lb(re %s%s)", amount, refSpec);
	}
	else if (base == M_E) {
	    nchar = _snprintf(buf, size, "ln(re %s%s)", amount, refSpec);
	}
	else if (base == 10) {
	    nchar = _snprintf(buf, size, "lg(re %s%s)", amount, refSpec);
	}
	else {
	    nchar = _snprintf(buf, size,
