    ...
    const char* const		variable)
{
    return
	cvNeedsParentheses(variable)
	? _snprintf(buf, max, "%g*(%s)", conv->scale.value, variable)
	: _snprintf(buf, max, "%g*%s", conv->scale.value, variable);
}
    ...
    const char* const		variable)
{
    ...
    return
	cvNeedsParentheses(variable)
	    ? _snprintf(buf, max, "(%s) %c %g", variable, oper, 
    ...
    const char* const		variable)
{
    ...
    return
	cvNeedsParentheses(variable)
	    ? _snprintf(buf, max, "%g*(%s) %c %g", conv->galilean.slope,
		variable, oper, fabs(conv->galilean.intercept))
	    : _snprintf(buf, max, "%g*%s %c %g", conv->galilean.slope, variable,
    ...
    const char* const		variable)
{
    return 
        conv->log.logE == M_LOG2E
            ? _snprintf(buf, max, "lb(%s)", variable)
            : conv->log.logE == 1
                ? _snprintf(buf, max, "ln(%s)", variable)
                : conv->log.logE == M_LOG10E
                    ? _snprintf(buf, max, "lg(%s)", variable)
                    : _snprintf(buf, max, "%g*ln(%s)", conv->log.logE, variable);
}
    ...
    const char* const		variable)
{
    return 
	cvNeedsParentheses(variable)
	    ? _snprintf(buf, max, "pow(%g, (%s))", conv->exp.base, variable)
	    : _snprintf(buf, max, "pow(%g, %s)", conv->exp.base, variable);
}
    ...
    const char* const		variable)
{
	...
	if (cvNeedsParentheses(buf)) {
	    nchar = _snprintf(tmpBuf, sizeof(tmpBuf), "(%s)", buf);
	}
	else {
