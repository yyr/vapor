    ...
    void*		data)
{
    ...
    if (text[0] == '/') {
        ...
        (void)_snprintf(buf, sizeof(buf), "%s/%s",
    ...
    const XML_Char**	atts)
{
	...
	if (stricmp(name, "unit-system") == 0) {
	    startUnitSystem(data, atts);
	}
	else if (stricmp(name, "prefix") == 0) {
	    startPrefix(data, atts);
	}
	else if (stricmp(name, "unit") == 0) {
	    startUnit(data, atts);
	}
	else if (stricmp(name, "base") == 0) {
	    startBase(data, atts);
	}
	else if (stricmp(name, "dimensionless") == 0) {
	    startDimensionless(data, atts);
	}
	else if (stricmp(name, "def") == 0) {
	    startDef(data, atts);
	}
	...
	else if (stricmp(name, "singular") == 0) {
	    startSingular(data, atts);
	}
	else if (stricmp(name, "plural") == 0) {
	    startPlural(data, atts);
	}
	else if (stricmp(name, "symbol") == 0) {
	    startSymbol(data, atts);
	}
	else if (stricmp(name, "aliases") == 0) {
	    startAliases(data, atts);
	}
	else if (stricmp(name, "import") == 0) {
...
declareXml(
    ...
    int		standalone)
{
    if (stricmp(encoding, "US-ASCII") == 0) {
	currFile->xmlEncoding = UT_ASCII;
    }
    else if (stricmp(encoding, "ISO-8859-1") == 0) {
	currFile->xmlEncoding = UT_LATIN1;
    }
    else if (stricmp(encoding, "UTF-8") == 0) {
    ...
    const char* const   path)
{
    ...
    if (file.fd == -1) {
        ...
        do {
            ...
            nbytes = _read(file.fd, buf, sizeof(buf));
    ...
    const char* const   path)
{
    ...
    if (parser == NULL) {
       ...
       // (void)memmove(base, dirname(base), sizeof(base));
		(void)memmove(base, drv, sizeof(base));
        base[sizeof(base)-1] = 0;
