			  ...
			  JDIMENSION in_rows_avail)
{
  ...
  while (my_main->cur_iMCU_row < cinfo->total_iMCU_rows) {
    ...
    if (my_main->rowgroup_ctr < DCTSIZE)
			  ...
			  JDIMENSION in_rows_avail)
{
  ...
  boolean writing = (my_main->pass_mode != JBUF_CRANK_DEST);

  while (my_main->cur_iMCU_row < cinfo->total_iMCU_rows) {
    /* Realign the virtual buffers if at the start of an iMCU row. */
    if (my_main->rowgroup_ctr == 0) {
      for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
	   ci++, compptr++) {
	my_main->buffer[ci] = (*cinfo->mem->access_virt_sarray)
	  ((j_common_ptr) cinfo, my_main->whole_image[ci],
	   my_main->cur_iMCU_row * (compptr->v_samp_factor * DCTSIZE),
	   (JDIMENSION) (compptr->v_samp_factor * DCTSIZE), writing);
      }
      /* In a read pass, pretend we just read some source data. */
      if (! writing) {
	*in_row_ctr += cinfo->max_v_samp_factor * DCTSIZE;
	my_main->rowgroup_ctr = DCTSIZE;
      }
    }
    ...
    if (my_main->pass_mode != JBUF_SAVE_SOURCE) {
      if (! (*cinfo->coef->compress_data) (cinfo, my_main->buffer)) {
	/* If compressor did not consume the whole row, then we must need to
	 * suspend processing and return to the application.  In this situation
	 * we pretend we didn't yet consume the last input row; otherwise, if
