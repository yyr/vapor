...
//
class VDF_API NetCDF : public VetsUtil::MyBase {
public:
 NetCDF();
 ...
 //! \brief NetCDF API interface
 //! 
 //! A NetCDF data variable
 //
 class Variable {
 public:
	Variable();

	//! Constructor for NetCDF::Variable class
	//!
	...
	//
	bool Match(const NetCDF::Variable &v) const;
 ...
 int OpenRead(string path, const NetCDF::Variable &variable);
 ...
 void GetVariables(vector <NetCDF::Variable> &variables) const {
 ...
 VDF_API friend std::ostream &operator<<(std::ostream &o, const NetCDF &nc);
